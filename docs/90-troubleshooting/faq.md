...
It's designed for students and users wanting to learn Linux, tiling WMs, and terminal workflows.
...
**Always backup** before installing on an existing system.

See: [Installation on Existing Arch](../01-getting-started/installation-existing-arch.md)

### Can I try it without installing?

Sort of. You could:
1. Install Arch in a VM
...
You'll need to adapt `~/.bashrc` settings to `~/.zshrc`.
...
Hyprland supports touchscreens, but sokratOS keybind-focused workflow is optimized for keyboard/mouse.
